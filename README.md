...
Messages without `protocolVersion` are treated as version 1 during the initial migration. Newer unsupported versions are rejected. The version 1 asset-selection success payload permits missing `key` and `contentType` because the current parent application does not always send them.
