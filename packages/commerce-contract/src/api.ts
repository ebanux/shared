...
export const commerceApiSuccessSchema = <T extends z.ZodTypeAny>(result: T) => z.object({
  contractVersion: z.literal(COMMERCE_CONTRACT_VERSION),
  type: z.string().min(1),
  result,
}).strict();
...
export function encodeCatalogQuery(input: Partial<CatalogQuery>): URLSearchParams {
